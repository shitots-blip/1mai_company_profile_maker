...
// 遷移前にキャプションを消す（遷移後に古いキャプションが残らないよう）
...
// ナビゲーション後にキャプションを復元（clearCaption後は復元しない）
...
// キャプションを消してから遷移し、新ページで落ち着いてから返る
async function next(label = '次へ') {
  await page.waitForTimeout(600)
  await clearCaption()                                          // 遷移前にキャプション消去
  await tap(page.locator(`a:has-text("${label}")`).last())
  await page.waitForLoadState('networkidle')
  await page.waitForTimeout(500)                               // 新ページが描画されてから返る
}
...
// 2. STEP1 主な営業先：キャプションはこのページに表示
await caption('② 質問にタップで答えるだけ')
await page.waitForTimeout(1200)
await fakeSelectChip('個人住宅オーナー')
await page.waitForTimeout(800)
await next()  // clearCaption → タップ → STEP2 へ

// 3. STEP2〜7：各ページでキャプション表示 → next() で消してから遷移
const stepCaptions = [
  '対応サービスを選択', '対応する建物を選択', 'お客様の声を選択',
  '大切にしていることを選択', '会社の基本情報を入力', '代表者情報を入力',
]
for (const cap of stepCaptions) {
  await caption(`② ${cap}`)           // このページのキャプション
  await page.waitForTimeout(1400)
  await next()                          // clearCaption → 次ページへ
}

// 4. 写真追加画面
await caption('③ 写真はあとからでもOK')
await page.waitForTimeout(1800)
await next('写真なしで進む')           // clearCaption → 確認画面へ

// 5. 確認画面：スクロールして作成ボタンをタップ
...
// 8. 修正フロー：キャプション → 消去 → 修正画面へ遷移 → 新キャプション
await clearCaption()
await page.waitForTimeout(200)
...
await tap(editLink)
await page.waitForLoadState('networkidle')
await page.waitForTimeout(400)          // 新ページ描画を待ってからキャプション
